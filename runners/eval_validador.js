...
async function rodar(cliente, estado, dono, visao, casosEmbaralhados, nRodadas, linhasLog) {
  ...
  for (let r = 1; r <= nRodadas; r++) {
    for (let i = 0; i < casosEmbaralhados.length; i++) {
      ...
      const prompt = Rei.montarPromptValidador(visao, c.ordem);
      let cru = "";
      try { cru = await cliente.gerar(prompt); }
      catch (e) { cru = ""; linhasLog.push(`ERRO rede r${r} ${c.id}: ${e.message}`); }
      const v = extrairVeredito(cru);
...
(async function main() {
  ...
  console.log(`\n== eval do validador | ${cliente.nome} | ${rodadas} rodada(s) | seed ${seed} ==`);
  ...
  const resumo = [
    "",
    `== RESUMO | ${cliente.nome} | ${rodadas} rodadas | ${dur}s ==`,
  ...
  linhas.push(`prompt: montarPromptValidador (rei.js) — o mesmo do loop composto`);
  linhas.push("");
  for (const c of casos) {
  ...
  const nome = `evalval_${nomeMod}_s${seed}${tag ? "_" + tag : ""}_${ts}.txt`;
  ...
  const linha = [ts, cliente.nome, `rodadas=${rodadas}`, `seed=${seed}`,
