...
for(const f of process.argv.slice(2)){const P=carregar(f); const vivos=new Map(),vistos=new Set();
  ...
  reexec(P,(g,t,fase)=>{
    ...
    if(fase==="pre") estoquePre=Object.fromEntries(g.aldeias.map(a=>[a.id,a.recursos.madeira]));
    if(fase==="pos"){ for(const m of g.movimentos){ if(vistos.has(m.id)) continue; vistos.add(m.id);
        ...
        const p=E.preverCombate(g,m.tropas,a); if(!p.atacanteVence) continue;
        ...
        const pior=(E.forcaDefesa(g,a)+lanc*g.config.tropas.lanceiro.def)*bonus;
