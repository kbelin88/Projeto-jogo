...
const teto = n.teto_tropas; // v3: teto conta UNIDADES, nao forca
...
const endurOk = neutras3.every((a) => {
  ...
  return a.tropas[a.tipo] === esperado(ini.get(a.id)) && soDoTipo;
});
checa(`endurece +${inc} a cada ${intervalo} turnos, do SEU tipo, respeitando o teto ${teto}`, endurOk);

// prova explicita do TETO: ninguem passa do teto alem de um passo (em unidades)
const tetoOk = neutras3.every((a) => Engine.contarTropas(a.tropas) <= teto + inc);
checa(`nenhuma neutra ultrapassa o teto (${teto}) alem de um passo`, tetoOk);

const ex = neutras3[0];
console.log(`     ex.: neutra #${ex.id} ${ini.get(ex.id)} -> ${ex.tropas[ex.tipo]} ${ex.tipo}(s) (tropas ${Engine.contarTropas(ex.tropas)})`);
